...
export default function Notifications() {
  ...
  return (
    <div className="col-md-10 col-sm-9 vh-100 overflow-auto mt-0">
      <p>
        <button className="btn btn-outline-primary mt-2" type="button" data-bs-toggle="collapse" data-bs-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
          make a notification
        </button>
      </p>
      { addLoading ? <Spinner/> :
            ...
            <input name='image' onChange={(e) => { 
              ...
              if (files && e.target.files?.length !== 0  ) {
                ...
                if (imageFiles.length > 0) {
                  setNotification({ ...notification, [e.target.name]: e.target.files[0] }) 
                }
              }
              }} accept="image/*" type="file" id='upload-notification-image' hidden />
          ...
          loading ? <Spinner /> : allNotifications.length > 0 && allNotifications.map((x) => {
            return (
              <div className="set-notifications my-5 mx-5 pb-2 row">
                ...
                {x.image?.url && <img className='col-md-2 m-2 notification-image' src={x.image?.url} alt="" />}
              </div>
            )
          })
        }
